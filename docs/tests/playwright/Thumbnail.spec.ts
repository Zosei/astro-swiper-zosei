...
test.describe(`${_testName} Tests`, () => {
  test(`Swiper ${_testName}`, async ({ page }) => {
    ...
    const swiperContainerThumbnail = await testSwiperContainer(
      page,
      _testName,
      ".myswiper-thumbnail",
    );
    ...
    const slidesThumbnail = await testSwiperSlides(
      swiperContainerThumbnail,
      _testName,
    );

    // Assert active slides are the same
    const activeMain = await getActiveSlideIndex(slidesMain);
    const activeThumbnail = await getActiveSlideIndex(
      slidesThumbnail,
      "swiper-slide-thumb-active",
    );
    expect(
      activeMain,
      `Initial indexes do not match:  ${activeMain} !== ${activeThumbnail} `,
    ).toBe(activeThumbnail);
    ...
    const activeThumbnail2 = await getActiveSlideIndex(
      slidesThumbnail,
      "swiper-slide-thumb-active",
    );
    expect(
      activeMain2,
      `Indexes do not match after navigation: ${activeMain2} !== ${activeThumbnail2}`,
    ).toBe(activeThumbnail2);
    ...
    await expect(async () => {
      ...
      const activeThumbnail3 = await getActiveSlideIndex(
        slidesThumbnail,
        "swiper-slide-thumb-active",
      );
      expect(
        activeMain3,
        `Indexes do not match after thumbnail click: ${activeMain3} !== ${activeThumbnail3}`,
      ).toBe(activeThumbnail3);
    }).toPass({ timeout: 2000 });
  });
});

...
async function testSwiperContainer(
  page: Page,
  testName: string,
  swiperClass: string = ".swiper",
): Promise<Locator> {
  const swiperContainer = page
    .locator(`#test-${testName} ${swiperClass}`)
    .first();
...
async function getActiveSlideIndex(
  slides: Locator,
  className: string = "swiper-slide-active",
): Promise<number> {
  for (let i = 0; i < (await slides.count()); i++) {
    const slide = slides.nth(i);
    const classAttr = await slide.getAttribute("class");
    if (classAttr?.includes(className)) {
      const slideIndex = await slide.getAttribute("data-swiper-slide-index");
...
async function _testSwiperAutoplay(
  swiperContainer: Locator,
  swiperSlides: Locator,
  autoplayDelay: number,
) {
  const initialIndex = await getActiveSlideIndex(swiperSlides);

  // Wait for autoplay to advance to a different active slide index
  await expect(async () => {
    const currentIndex = await getActiveSlideIndex(swiperSlides);
    expect(currentIndex).not.toBe(initialIndex);
  }).toPass({ timeout: autoplayDelay + 4000 });
}

async function _testSwiperPagination(
  swiperContainer: Locator,
  swiperSlides: Locator,
) {
  ...
  await expect(bullet3).toHaveClass(
    /(^|\s)swiper-pagination-bullet-active(\s|$)/,
  );
...
async function _testSwiperNavigation(
  swiperContainer: Locator,
  swiperSlides: Locator,
) {
...
async function testSwiper(config: Config) {
  test(`Swiper ${config.testName}`, async ({ page }) => {
    ...
    expect(nSwiperErrors, `Expected 0 error, but found ${nSwiperErrors}`).toBe(
      0,
    );
  });
}
